        ...
        <section className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-all">
          ...
          <div className="p-8">
            <div className="flex items-center gap-4 mb-8 bg-brand-50/30 p-4 rounded-xl border border-brand-100">
              ...
              <div>
                <div className="text-[16px] font-bold text-gray-900">{data.contact.firstName} {data.contact.lastName}</div>
                <div className="text-[13px] text-brand-600 font-medium">{data.contact.jobTitle}</div>
              </div>
            </div>
            <div className="space-y-0.5">
              <DataRow label="First Name" value={data.contact.firstName} />
              <DataRow label="Last Name" value={data.contact.lastName} />
              <DataRow label="Job Title" value={data.contact.jobTitle} />
              <DataRow label="Phone Number" value={data.contact.phone} />
              <DataRow label="Work Email" value={data.contact.email} />
            </div>
          </div>
        </section>
          ...
          <div className="p-8 flex flex-col h-full">
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-brand-50/40 p-4 rounded-lg border border-brand-100">
                ...
                <div className="text-xl font-black text-brand-900">{data.workforce.totalEmployees}</div>
              </div>
              <div className="bg-blue-50/40 p-4 rounded-lg border border-blue-100">
                ...
                <div className="text-xl font-black text-blue-900">{data.workforce.usHqEmployees}</div>
              </div>
            </div>

            <div className="space-y-4 flex-1">
              <DataRow label="HQ City" value={data.workforce.hqCity} />
              <div className="flex py-3 border-b border-gray-50 last:border-0 items-start">
                ...
                <div className="w-1/2 flex flex-wrap gap-1.5">
                  {data.workforce.otherUsCities.map(city => (
              ...
              <DataRow label="Other Countries" value={data.workforce.otherCountries.join(', ')} />
              <DataRow label="Open Jobs" value={data.workforce.openJobs} />
            </div>

            <div className="mt-8 pt-6 border-t border-gray-100">
              <a href={data.workforce.linkedInUrl} target="_blank" rel="noopener noreferrer" className="text-brand-600 font-bold hover:underline flex items-center gap-2 text-[14px]">
                Go to LinkedIn ↗
              </a>
            </div>
          </div>
          ...
          <div className="p-8 flex flex-col h-full">
            <div className="flex items-center gap-6 mb-10">
              <div className="flex flex-col items-center justify-center bg-gray-900 text-white w-20 h-20 rounded-xl shadow-lg">
                <span className="text-2xl font-black leading-none">{data.glassdoor.overallRating}</span>
              ...
              <div className="flex-1">
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <span className="text-[14px] font-bold text-gray-900">{data.glassdoor.benefitsRating}</span>
                </div>
                <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-brand-500 rounded-full" style={{ width: `${(data.glassdoor.benefitsRating / 5) * 100}%` }}></div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6 flex-1">
               ...
               <div className="flex items-center justify-between">
                 ...
                 <StarRating rating={data.glassdoor.healthInsuranceRating} />
               </div>
               <div className="flex items-center justify-between">
                 <span className="text-[13px] font-medium text-gray-500">Retirement Reviews</span>
                 <StarRating rating={data.glassdoor.retirementRating} />
               </div>
               ...
               <div className="pt-4 space-y-4">
                 <div className="flex items-center justify-between py-1">
                   ...
                   <span className="text-[15px] font-bold text-gray-900 pr-1">{data.glassdoor.overallReviews}</span>
                 </div>
                 <div className="flex items-center justify-between py-1">
                   <span className="text-[13px] font-medium text-gray-500">Benefits Reviews</span>
                   <span className="text-[15px] font-bold text-gray-900 pr-1">{data.glassdoor.benefitsReviews}</span>
                 </div>
               </div>
            </div>

            <div className="mt-8 pt-6 border-t border-gray-100">
              <a href={data.glassdoor.glassdoorUrl} target="_blank" rel="noopener noreferrer" className="text-brand-600 font-bold hover:underline flex items-center gap-2 text-[14px]">
