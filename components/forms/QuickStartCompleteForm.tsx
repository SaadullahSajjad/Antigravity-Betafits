...
const QuickStartCompleteForm: React.FC<Props> = ({ onSave, onSubmit, isSubmitting = false }) => {
    ...
    if (!pages || pages.length === 0) {
        console.error('[QuickStartCompleteForm] No pages found in form data');
        return (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
                <p className="text-red-600">Error: Form structure is invalid. Please contact support.</p>
                <p className="text-sm text-gray-500 mt-2">Pages: {pages?.length || 0}</p>
            </div>
        );
    }
    
    if (!currentPageData) {
        console.error('[QuickStartCompleteForm] No page data for current page:', currentPage);
        return (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
                <p className="text-red-600">Error: Page {currentPage + 1} not found. Please contact support.</p>
            </div>
        );
    }
    ...
    const progressPercent = totalPages > 0 ? Math.round((currentStep / totalPages) * 100) : 0;
...
export default QuickStartCompleteForm;
