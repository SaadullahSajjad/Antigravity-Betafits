...
export async function GET(request: NextRequest) {
  try {
    ...
    if (allFeedbackRecords && allFeedbackRecords.length > 0) {
      ...
      const filteredRecords = allFeedbackRecords.filter((record) => {
      ...
      if (sortedRecords.length > 0) {
        console.log(`[EmployeeFeedbackAPI] Sample feedback record fields:`, Object.keys(sortedRecords[0].fields));
        const sampleFields = sortedRecords[0].fields;
        console.log(`[EmployeeFeedbackAPI] All tier-related field values:`, {
        ...
        // Log overall-related fields from EE Pulse Surveys
        console.log(`[EmployeeFeedbackAPI] Overall-related field values from EE Pulse Surveys:`, {
          'Overall': sampleFields['Overall'],
          'Overall Rating': sampleFields['Overall Rating'],
          'Rating': sampleFields['Rating'],
          'Score': sampleFields['Score'],
          'Overall Score': sampleFields['Overall Score'],
        });
      }
      ...
      console.log(`[EmployeeFeedbackAPI] Fetched ${allStatsRecords?.length || 0} records from Company x Year table`);
      
      // Log first record to see what link fields are available
      if (allStatsRecords && allStatsRecords.length > 0) {
        console.log(`[EmployeeFeedbackAPI] Sample Company x Year record fields:`, Object.keys(allStatsRecords[0].fields));
        console.log(`[EmployeeFeedbackAPI] Sample Company x Year record link fields:`, {
        ...
        for (const linkFieldName of linkFieldVariations) {
          ...
          if (Array.isArray(linkField) && linkField.length > 0) {
            if (linkField.some((id: string) => String(id).trim() === String(companyId).trim())) {
              console.log(`[EmployeeFeedbackAPI] ✅ Found Company x Year record via field "${linkFieldName}"`);
              return true;
            }
          } else {
            // Handle single linked record ID
      ...
      console.log(`[EmployeeFeedbackAPI] Filtered to ${statsRecords.length} stats records for company ${companyId}`);
      
      let statsRecord: any = null;
      if (statsRecords.length > 0) {
        statsRecord = statsRecords[0];
        console.log(`[EmployeeFeedbackAPI] ✅ Using Company x Year record ID: ${statsRecord.id}`);
      } else {
        console.log(`[EmployeeFeedbackAPI] ⚠️ No Company x Year record found for company ${companyId}`);
        ...
        if (allStatsRecords && allStatsRecords.length > 0) {
          ...
          console.log(`[EmployeeFeedbackAPI] Available link fields in Company x Year:`, availableLinkFields);
          console.log(`[EmployeeFeedbackAPI] All fields in Company x Year:`, Object.keys(sampleRecord.fields));
        }
      }
      ...
      if (statsRecord) {
        ...
        console.log(`[EmployeeFeedbackAPI] Stats record ID:`, statsRecord.id);
        console.log(`[EmployeeFeedbackAPI] Stats record fields:`, Object.keys(statsFields));
        
        // Log ALL fields to see what's available
        console.log(`[EmployeeFeedbackAPI] ALL STATS FIELDS:`, JSON.stringify(statsFields, null, 2));
        ...
        const findFieldValue = (possibleNames: string[], defaultValue: any = null): any => {
          for (const name of possibleNames) {
            ...
            if (value !== undefined && value !== null && value !== '') {
              console.log(`[EmployeeFeedbackAPI] Found field "${name}" with value:`, value);
              return value;
            }
          }
          return defaultValue;
        ...
        console.log(`[EmployeeFeedbackAPI] Mapped values:`, {
        ...
        if (retirementValue !== null && retirementValue !== undefined && retirementValue !== '') {
          ...
          console.log(`[EmployeeFeedbackAPI] Using retirement from Company x Year: ${numericValue} → rounded to ${parsedRetirement}`);
        } else {
          console.log(`[EmployeeFeedbackAPI] Retirement not found in Company x Year, will be null`);
        }
        ...
        console.log(`[EmployeeFeedbackAPI] ✅ Final stats from Company x Year:`, stats);
        console.log(`[EmployeeFeedbackAPI] Retirement value in final stats:`, stats.retirement, `(should be 4 from Company x Year)`);
        ...
        console.log(`[EmployeeFeedbackAPI] ⚠️ No stats record found in Company x Year, calculating from responses...`);
        console.log(`[EmployeeFeedbackAPI] ⚠️ WARNING: Retirement should come from Company x Year table, not calculated!`);
        ...
        console.log(`[EmployeeFeedbackAPI] Calculated overall: ${overallSum} / ${responses.length} = ${calculatedOverall}`);
        ...
        console.log(`[EmployeeFeedbackAPI] ⚠️ Retirement will be null because Company x Year record was not found`);

        stats = {
          overall: calculatedOverall,
          responses: responses.length,
          nonMedical: responses.length > 0 ? nonMedicalSum / responses.length : 0,
          employeeCost: responses.length > 0 ? medicalCostSum / responses.length : 0,
        ...
        console.log(`[EmployeeFeedbackAPI] Calculated stats from responses (retirement is null - must come from Company x Year):`, stats);
      }
    }
