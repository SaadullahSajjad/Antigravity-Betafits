...
export default function QuickStartCompleteFormPage() {
    ...
    const handleSave = async (values: FormValues) => {
        ...
        for (const [key, value] of Object.entries(values)) {
            if (value instanceof File) {
                // Store file name for reference, but not the File object
                serializableValues[key] = { type: 'file', name: value.name, size: value.size };
            } else if (Array.isArray(value) && value.length > 0 && value[0] instanceof File) {
                // Store file names for multiple files
    ...
    const handleSubmit = async (values: FormValues) => {
        ...
        try {
                    ...
                    try {
                        ...
                        if (fileValue instanceof File) {
                            ...
                            if (uploadResponse.ok) {
                                ...
                                // Store the file URL or ID for submission
            ...
            // Now submit the form with processed values (files replaced with URLs/names)
