...
export default function AddNewGroupFormPage() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('form_recFVcfdoXkUjIcod_progress');
        if (saved) {
            try {
                JSON.parse(saved);
            } catch (e) {
    ...
    const handleSave = async (values: FormValues) => {
        localStorage.setItem('form_recFVcfdoXkUjIcod_progress', JSON.stringify(values));
    };
    ...
    const handleSubmit = async (values: FormValues) => {
        ...
        try {
            const response = await fetch('/api/forms/submit', {
                ...
                body: JSON.stringify({
                    formId: 'recFVcfdoXkUjIcod',
                    formName: 'Add New Group',
                    values,
                }),
            });
            ...
            if (response.ok) {
                localStorage.removeItem('form_recFVcfdoXkUjIcod_progress');
                setIsSuccess(true);
                setTimeout(() => {
    ...
    return (
        <div className="max-w-4xl mx-auto animate-in fade-in duration-500 py-12 px-4 sm:px-6 lg:px-8">
            ...
            <AddNewGroupForm onSave={handleSave} onSubmit={handleSubmit} isSubmitting={isSubmitting} />
        </div>
    );
}
