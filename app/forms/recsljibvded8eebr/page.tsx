...
export default function DocumentUploaderFormPage() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('form_recsLJiBVdED8EEbr_progress');
        if (saved) {
            try {
                JSON.parse(saved);
            } catch (e) {
    ...
    const handleSubmit = async (values: FormValues) => {
        ...
        try {
            ...
            if (response.ok) {
                localStorage.removeItem('form_recsLJiBVdED8EEbr_progress');
                setIsSuccess(true);
                setTimeout(() => {
    ...
    return (
        <div className="max-w-4xl mx-auto animate-in fade-in duration-500 py-12 px-4 sm:px-6 lg:px-8">
            <div className="mb-6 flex items-center gap-2 text-[13px] font-medium text-gray-500">
                ...
                <span className="text-gray-900">Document Uploader</span>
            </div>
            ...
            <DocumentUploaderForm onSave={handleSave} onSubmit={handleSubmit} isSubmitting={isSubmitting} />
        </div>
    );
}
