...
export default function NDAFormPage() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('form_recySUNj6jv47SOKr_progress');
        if (saved) {
            try {
                JSON.parse(saved);
            } catch (e) {
    ...
    const handleSave = async (values: FormValues) => {
        localStorage.setItem('form_recySUNj6jv47SOKr_progress', JSON.stringify(values));
    };
    ...
    const handleSubmit = async (values: FormValues) => {
        ...
        try {
            ...
            if (response.ok) {
                localStorage.removeItem('form_recySUNj6jv47SOKr_progress');
                setIsSuccess(true);
                setTimeout(() => {
    ...
    return (
        <div className="max-w-4xl mx-auto animate-in fade-in duration-500 py-12 px-4 sm:px-6 lg:px-8">
            <div className="mb-6 flex items-center gap-2 text-[13px] font-medium text-gray-500">
                ...
                <span className="text-gray-900">NDA</span>
            </div>
            ...
            <NDAForm onSave={handleSave} onSubmit={handleSubmit} isSubmitting={isSubmitting} />
        </div>
    );
}
