...
const Sidebar: React.FC<Props> = ({ currentView, onNavigate }) => {
  const [isCollapsed, setIsCollapsed] = useState(false);

  const navItems = [
    { id: 'home', name: 'Dashboard', icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
    { id: 'company-details', name: 'Company Details', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4' },
    { id: 'benefit-plans', name: 'Benefit Plans', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
    { id: 'benefits-analysis', name: 'Benefits Analysis', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
    { id: 'employee-feedback', name: 'Employee Feedback', icon: 'M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z' },
      ...
      <div className="p-6 flex-1 overflow-y-auto pt-10 px-4">
        <nav className="space-y-2">
          {navItems.map((item) => (
            ...
            >
              <svg 
                className={`w-5 h-5 flex-shrink-0 transition-colors ${currentView === item.id ? 'text-gray-900' : 'text-[#4b5563]/50 group-hover:text-[#4b5563]'}`} 
                fill="none" 
                viewBox="0 0 24 24" 
                stroke="currentColor"
              >
        ...
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3 bg-gray-50/50 p-2 w-full border border-gray-100'} rounded-md transition-all cursor-pointer group`}>
          ...
          {!isCollapsed && (
            <div className="flex flex-col min-w-0">
              <span className="text-[11px] font-semibold text-gray-900 truncate tracking-tight">Matt Prisco</span>
              <span className="text-[10px] text-gray-400 font-medium truncate">Betafits Advisor</span>
            </div>
          )}
        </div>
      </div>
